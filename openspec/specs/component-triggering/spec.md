...
This specification defines how APE-RV SHALL trigger Services and BroadcastReceivers at runtime using `am broadcast` and `am startservice`, with intent data read from the compact artifact's component trigger surface (`permission`, `intentFilters.actions`/`categories`, `authorities`, `hasTargetMethods`) — derived host-side from the static analysis, not parsed from it on the device. Triggering is probabilistic — on each exploration step, there is a `Config.componentPercentage` chance of triggering one component (round-robin). The trigger is a side-effect that does not consume a SATA step.
