...
Source of record: `rvsec/rv-android/docs/20260729_propostas_melhorias_e3.md` §0 — every decision below was fixed by the project author on 2026-07-29, **except the items tagged `2026-07-31`, which were decided here on evidence the adversarial verification produced after that session**: D11 (O4) is new, D1's threshold was re-selected, and D9 withdraws the mechanism the record named for A10. **All three were ratified by the author on 2026-07-31**, D9 and D11 as written and D1 with one amendment — the input-capable exemption, which restores §0's `EditText` carve-out on top of the re-selected threshold. This design resolves the implementation details the record left open.

There is **no deadline**. The jar feeds the decisive run that decides whether the LLM stays in the design, and the run is not on a clock. Group order below is a dependency and risk order — B1 and the LLM-path items are the ones the decisive run's LLM arm cannot be interpreted without — and nothing is scoped out on schedule grounds; A3 in particular is implemented, not cut.
...
- **Death rule** (author-fixed range k=2–3; **k=5 selected on re-measurement, 2026-07-31, and ratified the same day with the input-capable exemption restored**): a pair dies after **five** executions whose recorded outcome has `new_state=false`, the same threshold for every widget class the ban covers. A `new_state=true` execution neither counts toward death nor resets the accumulated count.

  **Input-capable targets are exempt at any strike count.** §0 exempted `EditText` by name, and no finite k reproduces an exemption — a threshold protects a widget for k executions, it does not protect it forever. The author's ratification therefore takes k=5 for the classes the threshold argument covers and keeps the carve-out for the one class that argument cannot reach. Its definition is the existing `ApePromptBuilder.INPUT_CLASS_NAMES` (`ApePromptBuilder.java:31-36`: `android.widget.EditText`, `android.widget.AutoCompleteTextView`, `android.widget.SearchView`, `androidx.appcompat.widget.SearchView`), read through `isInputClass(GUITreeNode)` (`:783`) — the set that already decides whether the prompt offers `type_text` and which widgets D4's fixTextEdit converts. **No second list** (P1): if the notion of "input-capable" is ever wrong, it is wrong in one place.

  Three consequences, stated because each is asserted by a scenario rather than left to be discovered:

  1. **Realized by not recording strikes**, not by filtering at the ban check. An exempt pair never enters the record, so there is no state to consult and nothing to keep in memory for it. This is the cheaper site and the more directly testable invariant.
  2. **It keys on the widget, not the event type**, so it covers the text-entry action D4 converts a click on that widget into. The exemption and fixTextEdit act on one widget set; a decision landing on an input widget is *transformed* rather than refused, and is never withdrawn by repetition.
  3. **It applies to the `matched` half only** — not a scoping choice but a property of the evidence: an `llm_tap` carries `matched_class=none` in 1,033 of 1,033 corpus occurrences, so no class-based exemption of any design can reach it.

  **What the exemption does to the measured share:** it can only remove pairs from the refused block, so the 27.5% below becomes an **upper bound** and the sub-30% ceiling holds a fortiori. **The exact post-exemption share was not re-derived** — the sweep was not re-run for a number that can only move in the safe direction, and the decisive run's `dead_pair` summary counter reports the realized value directly.

  The k selection replaces the earlier "one-strike with typed exceptions (Switch/CheckBox/RadioButton k=2, EditText never)" reading of the same author decision for the *stateful trio*; `EditText`'s exemption is not replaced but retained. It was made by sweeping k against the 84 `cal_a1` runs of `experimento-cal/iter0` — the same 70% LLM configuration the decisive run's LLM arm uses — reconstructing each run's LLM decision stream from the `[APE-STEP]` / `[APE-LLM-TEL]` / `[APE-OUTCOME]` join and replaying the ban rule over it.
  ...
  *Denominator note*: **the table's denominator is 6,500 decisions**, and it is only reachable with the 4 `cal_a1` smoke runs included (84 traces = 80 main at `timeout=300` + 4 smoke at `timeout=90`). The main-only denominator is 6,440. Holding the shipped-key numerator at 1,788, the main-only share is 27.8% rather than 27.5% — a shift of 0.3 pp that leaves k=5 inside the 30% ceiling and changes no conclusion. (A main-only *numerator* was not re-derived; the figure above is the bound obtained by holding it fixed, which is the conservative direction.) The denominator label is corrected here because every share in the table is computed against it.
  ...
  2. **k=5 subsumes the exception list for every class except `EditText`, which is why the list shrinks to the input-capable carve-out rather than disappearing.** The list would have granted `Switch`/`CheckBox`/`RadioButton` a threshold of k=2; k=5 grants them 5, so each ends up at least as protected with no class test at all. Those classes are *not* rare — at k=1 they account for **81 of 2,586** banned pairs (3.1%), with `Spinner` (10) and `CheckedTextView` (15) adding more — which is why "the list is inert" would have been the wrong argument; the right one is that k=5 already exceeds what the list offered them. The threshold additionally reaches what no class-name enumeration can: `Spinner` and other N-ary selectors (one coordinate, many options), Material/AppCompat subclasses whose simple names differ from the base classes (the corpus shows AndroidX simple names such as `LinearLayoutCompat`, `FloatingActionButton`, `CardView` reaching the tree, so the divergence is real), and Compose trees with no meaningful class name.

     `EditText` is the one entry the arithmetic does not absorb, because §0 exempted it outright and an exemption is not a larger k. It is retained, generalized to the four input-capable classes the prompt builder already recognizes. The residual cost is honest and small: the carve-out is a class-name test and inherits the same blind spot as the list it replaces — an input widget whose class name is none of the four is banned like any other — and it cannot protect `llm_tap`, which carries `matched_class=none` in 1,033 of 1,033 corpus occurrences.
...
- [B1 bans a pair that would have worked on the k-th try] → measured directly rather than argued: at k=5 under the shipped keys the ban would have refused 1,788 decisions in the calibration corpus, of which 9 (0.50%) had actually produced a new state. k=5 absorbs the class-based exception list for the stateful classes without weakening it — the list granted them k=2 and the threshold grants them 5 — and `EditText` keeps its exemption outright rather than being traded for a larger k. Both figures are pre-exemption and therefore upper bounds. The falsification gate catches a wrong call cheaply.
- [The input-capable exemption lets a genuinely dead input widget be re-emitted without limit] → accepted, and bounded by the mechanism next to it: D4 converts a click on an input widget into a text-entry action, so a repeat is not a repeat of the same executed action — the generated text differs. What the exemption does forfeit is the ban's protection against an LLM looping on one text field, and nothing in this change detects that. It is observable after the fact: an exempt widget's repeats stay in the executed stream instead of appearing under `dead_pair`, so a loop shows up as unproductive steps in the decisive run's telemetry rather than being silently absorbed.
...
- [A3 touches all 4 pick sites and could perturb selection] → the dedicated RNG test is the merge gate: same seed, A3 on and off, identical action sequence. A3 is implemented, not deferred — the schedule that once made it the designated cut no longer exists.
...
**None outstanding.** Three items in this change were decided here rather than in the source of record, and all three were **ratified by the author on 2026-07-31**:

- **D1's threshold** — ratified **with an amendment**. §0 fixes B1 as "one-strike com exceções por tipo (Switch/CheckBox/Radio cap k=2–3; EditText isento)"; this change proposed k=5 uniform with no exemption list, on a re-measurement over the shipped keys. The author took k=5 for the classes the threshold argument covers **and restored §0's `EditText` carve-out**, generalized to the four input-capable classes. The residual gap the original text flagged — "it does not cover EditText, which §0 exempted by name and which k=5 bans" — is closed. The criterion invoked to move k, refusal under 30%, is stated in this design and not in §0, and it survives the amendment because an exemption can only lower refusal.
- **D9's mechanism** — ratified as written. §0 names "shutdown hook idempotente"; this change withdraws it for teardown reordering and deletes the idempotence flag, on mechanical and checkable evidence that the hook recovers zero on the failure path that loses the dumps; §0's own figure for this item (165/880) is superseded. The **device-side sink** stays deliberately out of scope (D9's closing note) — a scoping call, not a refutation, and the one design that could reach the remaining 1.5%.
- **D11 (O4)** — ratified as written and retained in scope. It has no §0 entry at all; it was decided here on the verification's evidence.
