...
Group order is the deadline-driven priority order (decided 2026-07-29, source of record `rvsec/rv-android/docs/20260729_propostas_melhorias_e3.md` §0): B1 and the LLM-path fixes must be in the jar for the decisive run (implemented + tested by Friday 09:00, hard max Saturday 09:00). Group 16 (A3) is the **designated cut** if time runs out.
...
## 16. A3 — per-step counterfactual (**DESIGNATED CUT — optional-if-time; drop this whole group if the Friday window closes**) (~50–80 LOC + dedicated test)
