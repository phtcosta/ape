...
- [x] 17.4 Real smoke via rv-platform — **satisfied by the E3 decisive run instead of a separate smoke (author decision, 2026-08-02)**: the run is a strictly stronger instance of this task (120 runs per arm × 1800 s vs 2–3 APKs × ~5 min) and every gate below was measured on it. (a), (b), (d), (e), (f) PASS; (c) PASS with a residual of 74/576,739 broken lines (0.013% vs 0.45% pre-A8) caused by `\n` inside the action Name's `text=` — an interpolation site A8 did not cover; no field is lost (the tail continues on the next physical line). Evidence, commands and per-gate table: `rvsec/rv-android/docs/20260802_avaliacao_itens_ape.md` §7. Original scope, kept for the record (rv-android side, real SGLang server, 2–3 APKs × ~5 min), gates:
