...
- [ ] 4.1 `LlmRouter.mapToModelAction`: when `click`/`long_click` resolves (containment or snap) to an input-capable widget, do NOT return the bare click — convert to a text-entry action on that widget, text generated by APE's existing typed-input path (where = LLM coordinate, what = harness; no second LLM call)
- [ ] 4.2 JVM unit test where mockable: click-on-EditText yields a text-entry action, not a bare click; `type_text` answers unaffected
