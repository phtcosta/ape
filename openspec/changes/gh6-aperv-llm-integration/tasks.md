...
- [ ] 3.2 Implement system message with tool schema declaration: `click(x, y)`, `long_click(x, y)`, `type_text(x, y, text)`, `back()` — 4 tools total. Include MOP explanation: "Elements marked [DM] or [M] reach operations monitored by runtime verification specifications. Prefer exploring these when they haven't been visited yet." Use [DM] (direct monitored) and [M] (transitive monitored) compact notation matching rvsmart V17. (ref: `specs/llm-prompt/spec.md` — System Message requirement)
...
- [ ] 5.3 Hook stagnation LLM mode: when `graphStableCounter > graphStableRestartThreshold / 2`, try LLM via `_llmRouter.shouldRouteStagnation(graphStableCounter)` + `selectAction()`. If LLM returns action, use it and reset graphStableCounter to 0. If null, continue with normal stagnation/restart logic. (ref: `specs/exploration/spec.md` — SataAgent LLM Stagnation Hook; `specs/llm-routing/spec.md` — Stagnation LLM Mode)
...
- [ ] 6.5 Write `ApePromptBuilderTest`: widget list format with MOP markers, coordinate normalization, action history with results, text truncation, null MopData, empty history omitted, long_click in tool schema
