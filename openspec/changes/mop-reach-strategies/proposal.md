...
- `mop-guidance`: A′ 3-source union for `mopActivities`; FIX 2 desugared-lambda widget-flag recovery; FIX 3 handler-join diagnostics; `isWidgetlessSubstrate()` predicate; G-2 unit fix to the load-size budget check.
...
- **Code**: `MopData.java` (A′ 3-source union population incl. reachability-method-level; FIX 2 desugared-lambda widget-flag recovery in `deriveWidgetMopFlags` + a `className→hasReachingLambda` index in `parseReachability`; FIX 3 join diagnostics on the load line; `isWidgetlessSubstrate()`; G-2 unit fix), a new `MopFrontierPass` in `com.android.commands.monkey.ape.agent.scoring`, `SataAgent.selectTriggerCandidate` (ordering branch), `Config.java` (4 new flags + P4 comments). FIX 2/FIX 3 are consumer-side only — no producer/GATOR change.
