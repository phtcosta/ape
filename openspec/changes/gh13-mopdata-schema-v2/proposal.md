...
2. **T1.2** OPTIONSMENU-aware menu-open boost: precomputed `Set<String> activitiesWithMopOptionsMenu` populated during `MopData.load` by scanning `Window`s of `type="OPTIONSMENU"` whose `name="<activity>#OptionsMenu"` carries at least one widget with `directMop || transitiveMop`. Scored against the `MODEL_MENU` action. Single biggest cryptoapp win (100% of cryptoapp's MOP-reachable widgets live in the OPTIONSMENU window).
...
Every documented field in the post-gh60 JSON has a typed Java home. No silent drops of known fields. Forward compat for *unknown* fields preserved via `default: skipValue()`.
...
- **`ComponentInfo` (extended)**: `className`, `componentType` (derived from the parent dict key in gh60 — `activities`/`receivers`/`services`/`providers`), `isMain`, `exported`, `intentFilters[]` (structured with `actions[]` AND `categories[]`), `authorities` (providers), `reachesTarget` (read from JSON, not hardcoded), `targetMethods[]`.
- **`ComponentInfo.IntentFilter`**: `actions[]`, `categories[]`.
...
- `src/main/java/com/android/commands/monkey/ape/utils/MopData.java` — full rewrite (~850 LOC). New nested POJOs (`Window`, `Widget`, `Listener`, `Transition`, `TransitionEvent`, `ReachabilityClass`, `ReachabilityMethod`). Five-pass loader. Precomputed `activitiesWithMopOptionsMenu` set. Three-arg `load(path, expectedPackage, expectedMainActivity)` overload. **Carries the bug fix** (read `reachesTarget` / `directlyReachesTarget` keys in `parseMethod` and `parseComponentArray`).
...
- `src/test/` — new test classes / methods: `MopDataTest` (20), `ComponentInfoTest` (4), `MopScorerTest` extensions (5), `ApeFuzzerInputTypeTest` (6), `StatefulAgentTriggerTest` (5), `ApePromptBuilderTest` extensions (6). Total **+46 unit tests** on top of the existing ~145. Includes the **bug-fix regression test** `testWidgetDirectMopDerivedFromGh60Targets` — pins `Widget.directMop=true` on `menu_item_message_digest` (would have caught the legacy-keys bug at CI time).
