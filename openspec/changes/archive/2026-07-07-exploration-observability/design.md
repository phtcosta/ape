> **SUPERSEDED — attribution model (2026-07-02).** The branch-level `decision_source` rule described in the Architecture/Design sections below (attribute over the whole `EARLY_STAGE`/`EPSILON_GREEDY` branch; tie order `MOP>WTG>Menu>Coverage`; no `Form`; no `clock=`) was the delivered group-1 pass and is NOT the target to implement. It is SUPERSEDED by the authoritative delta `specs/action-selection/spec.md` and tasks §5.1–5.5: attribution is scoped to the individual priority-consuming pick sites (`randomlyPickAction` `:487`, `randomPickWithPriority` `:1072`, `selectUnvisitedMopTarget` `:476`); Back/Menu-unvisited, `greedyPickLeastVisited`, and navigation stay `SATA`; the precedence is `MOP>WTG>Menu>Form>Coverage` (adds `DecisionSource.Form`, bump `ModelActionTest` `values().length` 9→10); and the `[APE-STEP]` line gains a `clock=<epochMillis>` field (`.trace`/Logger only, never logcat). Implement §5, not the superseded narrative below. (Mirrors `tasks.md:8`.)
