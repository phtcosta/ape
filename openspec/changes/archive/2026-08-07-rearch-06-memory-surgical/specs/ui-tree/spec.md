...
- **INV-TREE-13**: After `GUITreeBuilder.release(tree)` returns, no static `GUITreeBuilder` cache SHALL hold an entry keyed by that tree or by any `GUITreeNode` of that tree, under any `Naming`; and no operation of the same release cycle SHALL touch the released tree afterwards (release is the cycle's last use of the tree). Cache clearing MUST be decision-neutral: same seed ⇒ identical action sequence with and without the entries present. This is verified by the release-cycle unit tests and by the recompute-equality property above, not by the rearch-01 parity goldens — the oracle builds no GUITrees and releases none, so it cannot observe this property either way.
