...
**Verdict at 2026-08-04: group 3 (V11) is BLOCKED**, exactly as this task anticipated. Consumer A1 is still live and still re-resolves every deep record through the rich `GUITreeAction`, so the snapshot shape cannot be applied without breaking it. Re-sequencing: group 3 runs after `rearch-04-step-ndjson-telemetry` deletes A1. Groups 2 and 4 proceed — neither depends on stages 2/4.

**Re-verified 2026-08-05: the block is cleared.** `rearch-04` task 7.2 (commit `6c495eea`) deleted `Model.saveActionHistory`, `StatefulAgent.saveActionHistory()` and `action-history.log`. The gate was re-run on this branch's HEAD with a `python3` scan of `src/main/java` rather than `grep`, because the claim is an absence: the pattern `saveGraph|readGraph|sataModel.obj|saveActionHistory|action-history.log` matches only `KeyOwnership.java:263` and `:269`, the two retired-key abort messages the task's `grep -v KeyOwnership` excludes. Stage 2 and stage 4 have both landed; consumer A1 no longer exists. The B6 row above (`ActionRecord.resolveModelAction`, alive at `c73067a` only because stage 4 had not landed) dies in group 3 as that row predicted. Group 3 proceeds under the design below, unamended — task 1.2's gate was already satisfied on its own terms, so no re-audit was owed.
