...
**Second, distinct static-analysis artifact — ICC edges not modeled (intra-component reachability).** Activity-level MOP reachability is *intra-component*: the producer call graph does not traverse `startActivity(Intent)` inter-component edges, so a pure **navigation-hub** activity that only launches other activities is correctly **not** MOP-flagged. cryptoapp's `MainActivity` is exactly this (`MainActivity: 0` reaching methods, `startActivity(new Intent(this, {Cipher,MessageDigest,Cryptography}Activity.class))` only) — the MOP operations live in the three activities it launches. This is *not* a defect for our levers: E-mín / A′ / `MopFrontierPass` intentionally target the activities that **host** MOP (all three correctly flagged), and E-mín's launcher excludes the main activity anyway (`!isMain`). Launching a MOP-hosting activity directly at stagnation — bypassing the hub — is the desired behaviour. Consequence for the device smoke: cryptoapp cannot *distinguish* E-mín from round-robin (all three eligible, non-main, exported candidates are MOP-reaching, so the two orderings coincide); the discriminative preference (a mix of MOP and non-MOP eligible candidates) is proven by the `selectTriggerCandidate` JVM tests, not by this app.
...
- When true, the walk considers eligible **MOP-reaching** candidates **before** eligible non-MOP-reaching ones. A candidate is MOP-reaching iff `MopData.activityHasMop(className)` — the reachability-augmented set (INV-MOP-27, the same truth `MopFrontierPass` and A′ consume), **not** the component-level `ComponentInfo.reachesTarget`. This matters because the producer's `components.activities[].reachesTarget` false-negatives lambda-triggered activities: on cryptoapp all 4 activities report `components.reachesTarget=false` while `reachability[]` shows CipherActivity=2, MessageDigestActivity=1, CryptographyActivity=13 reaching methods — so keying on `reachesTarget` would never fire the preference on exactly the apps E-mín targets, reproducing the very lambda-gap bug A′/FIX-2 fix. Implemented as a stable two-pass over the eligible set (MOP-reaching group first, each group in round-robin order), so the selection stays **deterministic and reproducible** under a fixed seed — no randomness, no set-iteration-order dependence.
- When false (default), the walk is exactly `activity-frontier`'s single-pass round-robin — behaviour unchanged, `activityHasMop` not consulted.
...
The seam takes the MOP-membership as a `Set<String> mopActivities` parameter (null ⇒ flag-off round-robin, MOP not consulted) so it stays a pure, JVM-testable function decoupled from the live `MopData`; the production call site passes the runtime `mopActivities` set.
...
| Unit (E-mín) | `triggerMopFirst=true` picks the `activityHasMop`-true eligible candidate first (via the `mopActivities` set param, not `ComponentInfo.reachesTarget`); determinism under fixed order; flag off = round-robin unchanged | `selectTriggerCandidate` seam test |
