...
- [x] 4.3 Device-smoke characterization via cmpft3 (`rvsec/rv-android/docs/20260708_analise_efeito_changes_cmpft3.md` §4, §10). Verdict **PASS (model purity) / FAIL (restart storm)**. Evidence: `[APE-RV] Tree/package mismatch` fired **29×** (29 runs, ≤1/run, 19 APKs — the transient a11y-capture race: window says app, tree still says launcher/ime/system); `NexusLauncher` states recorded in the model went **cmpft2=25 (16 apps) → cmpft3=0** despite cmpft3 recording MORE states overall (200,789 vs 187,812), with 3 apps causally proven (pachli, password.monitor, mrmcd.schedule). The **relaunch-cycle-drop clause does NOT hold**: fossify.messages went 103/101/101 → 101/100/**117** (worse, guard fired 0× there); the storm reduction concentrates on apps that do NOT fire the guard → not attributable to #3 (mechanically the guard refetches one tree, it does not break the relaunch loop). Per §11: **reescope the claim** — keep the model-purity result, DROP the "reduces restart storm" promise. Controlled per-arm validation deferred to the gh74 fair-test (`PAMunb/rvsec#74`).
