...
- **No production behavior change.** Test infrastructure, plus one behavior-identical line:
  `SataAgent.egreedy()` reads the agent RNG through the overridable `getRandom()` instead of
  `ape.getRandom()` directly, which is what makes the epsilon-greedy rung capturable at all
  (owner decision 2026-08-03; recorded in INV-ORA-01 and design D5).
...
- **One line** in `src/main/java/` — the `egreedy()` RNG seam, behavior-identical and carved out explicitly in INV-ORA-01. **Zero changes** to the `ape.properties` surface, the CLI surface, or the Python side (rv-android untouched).
