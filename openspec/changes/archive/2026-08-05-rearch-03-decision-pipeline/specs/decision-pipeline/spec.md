...
- **INV-DP-12**: No stage, engine, LLM unit, scoring pass, `MopScorer`, or the agent and model classes the decision path runs through (`SataAgent`, `StatefulAgent`, `State`) SHALL read `Config` at decision time; every behavioral parameter of the decision path SHALL be injected from the `RunSpec` at assembly. "Read `Config`" covers **both source forms** — a qualified `Config.<field>` and a bare identifier behind `import static ...Config.<field>` — and the **ambient form**, a plan value fetched at a decision site through `RunContext.current()` rather than received at assembly; the three are the same defect and the second is the one a `Config.` grep passes over. Reaching the run context at *assembly* — a constructor deriving its parameter object once — is injection, not a violation, and is how the parameters get there. (Documented residue: `State.getActions()`'s `modelMenuEnabled` structural gate — see design D9.)
