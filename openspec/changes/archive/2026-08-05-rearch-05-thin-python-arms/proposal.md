...
- **The rv-android side is recorded as delivered by `gh95`, not re-planned here.** This change's remaining rv-android obligation was coordination: confirm `gh95` closes, and confirm its group 7 marks the reserved counterpart task satisfied. **Both were discharged read-only on 2026-08-05** (tasks 1.1–1.3): `gh95` is 56/57 with only the owner sign-off open, its task 7.1 gate is green, and its 7.7 is ticked carrying exactly the reading task 1.3 prescribes — *"There is nothing to tick over there"*. What remains is not an obligation on this change but a request to the owner (task 1.3a), and no task below edits that repository.
