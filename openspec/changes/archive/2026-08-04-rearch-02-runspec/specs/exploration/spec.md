...
- **INV-EXPL-03** is **REMOVED**. It documented that `sataModel.obj` contains a serialized `Model` (not a `Graph`) — the write half of a persistence protocol verified broken by construction (writes `Model`, reads with a `Graph` cast, swallows the `ClassCastException`, returns an empty graph). The protocol is deleted by this change (report Sec. 8: deliberately dissolved; R1/R3 — clean runs, no read-back). Substitute: **INV-RUN-07** (`run-spec` capability) — no artifact produced by a previous run is read by the explorer, so there is no model/graph deserialization path, no resume, and no read-back of trace or telemetry. The substitution widens rather than swaps. INV-EXPL-03 constrained the *content* of one file this change deletes, and at that level nothing replaces it: there is no serialized model artifact left to constrain. INV-RUN-07 constrains the *class* that file belonged to, which is the property worth carrying forward — a reintroduced serializer would violate the invariant before it could produce a file whose content anyone would need to describe.
