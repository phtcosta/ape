...
|-------------|---------------|------|
| run-spec: Single Resolution at Bootstrap (INV-RUN-01) | `RunSpec.resolve` called once in `Monkey.run` before `MonkeySourceApe` | `RunSpecResolveTest` (immutability, input-map isolation); `RunContextTest` (second `initialize` throws) |
| run-spec: Feature Dependency Validation (INV-RUN-05) | `Feature` enum metadata + `RunSpec` validator | `FeatureDerivationTest`; `RunSpecResolveTest` (inert vs. non-neutral) |
| run-spec: key ownership is total | `KeyOwnership` + `Feature` metadata | `KeyOwnershipTotalityTest` (roster scanned from `src/main`, minimum-yield floor) |
| run-spec: Presets in the Jar | `Presets.resolve` | `PresetsTest` (content vs. the four arm vectors) |
| run-spec: Explicit-Key Resolution (no preset key) | `RunSpec.resolve` default path | `RunSpecCompatTest` (fixtures = `_push_properties` output of the four campaign arms + `ape_pure`) |
| run-spec: Total Fail-Fast (INV-RUN-02) | validator + `Monkey.run` abort path | `RunSpecAbortTest` (unknown key / bad type / dependency / combination / retired key); the abort *composition* in `Monkey.run` is device-side and deferred to the coordinated end-to-end smoke (D-13) |
| run-spec: RUN_START echo (INV-RUN-03/04) | `RunSpecEcho` + minimal JSON serializer | `RunSpecEchoTest` (content reconstructs the arm; single line; escaping; bootstrap source order) |
| run-spec: Build Provenance Stamp | `BuildInfo` template + pom plugins | `BuildInfoTest` + dex `strings` check |
| run-spec: RunContext ownership (D-12) | `RunContext` holds the spec, the run identity and the seeded stream | `RunContextTest` (once-only, runId shape, seeding, `installForTest`) |
| run-spec: no `/sdcard` input but `ape.properties` (INV-RUN-06) | the three static readers deleted | `DeviceInputChannelAbsenceTest.noCodePathReadsABehavioralFileFromSdcardOtherThanApeProperties`, with two anti-vacuity guards |
| run-spec: single seeded stream (INV-RUN-08) | `RunContext.initialize` seeds `RandomHelper`; `StringCache` draws from it | `DeviceInputChannelAbsenceTest` — `noUnseededGeneratorSurvivesInProductionCode` (`ThreadLocalRandom`), `noOtherUnseedableGeneratorSurvivesInProductionCode` (`Math.random`), `theOnlyUnseededRandomIsTheOneTheContextReplaces` (exactly one bare `new Random()`, `RandomHelper`'s); `StringCacheSeededTest` |
| run-spec: no read-back of a previous run (INV-RUN-07) | `Graph.readGraph`, `--ape-model` and `ape.modelFile` deleted | `DeviceInputChannelAbsenceTest.noCodePathReadsBackAnArtifactOfAPreviousRun` (no `ObjectInputStream`/`readObject`/`readGraph`), plus compile enforcement and `RunSpecAbortTest.everyRetiredKeyAbortsWithItsOwnReason` |
| exploration: Strategy Selection (validated) | `createAgent(spec)` rewrite | `RunSpecAbortTest` (`--ape bfs` aborts) |
| exploration: Output Persistence (saveGraph removed, INV-COV-10 restated) | `StatefulAgent.tearDown` chain edit | `StatefulAgentCoverageDumpOrderTest` and `StatefulAgentTearDownTest` (both existing, both updated by task 5.5) |
| model: no XPath injection | delete the `xpathaction` package and the `StatefulAgent` branch | compile-enforced; `DeviceInputChannelAbsenceTest` covers the `/sdcard` channel |
| heuristic-input: seeded generation (INV-INP-07) | `StringCache.nextString` on `RandomHelper` | `StringCacheSeededTest` |
| ui-tree: no XPathlet overlay | delete `GUITreeBuilder` static overlay block | compile-enforced; `DeviceInputChannelAbsenceTest` (`/sdcard/ape.xpath`) |
| scoring-pipeline: kill-switch removed | delete registries + `forceApePureModeInto` + `apePureMode` field | `KeyOwnershipTotalityTest` (there is no registry left to keep complete); `RunSpecAbortTest.theKillSwitchAbortsAtEitherValue` |
...
**The roster as measured at apply time**: **25 features** — exactly the table above — and **121 declared keys**: 59 base, 49 feature-owned, 3 resolver-owned, 10 retired. The "113 fields" of the Context section counts `Config`'s public static fields; the key roster is a different number and is now the one that matters, because ownership is asserted over keys rather than over fields.

The split moved once during the change and the total did not, which is the informative part. Group 1 measured 60 base and 9 retired; group 5 retired `ape.saveStates` (D-10), and because that key was already declared as a base key, retiring it **moved** a key from one column to the other rather than adding one. A retirement that raised the total would mean a key nobody had owned — exactly what the totality test exists to prevent.

Two of the 59 base keys were found by the totality scan rather than by this design, and both are read in production today: **`ape.baseNaming`** (`NamingFactory.java:953`, whose fallback is the caller's current naming, so the jar has no single default for it) and **`ape.nopActionThrottle`** (`Action.java:48`, default 1000). Neither appears in any artifact of this re-architecture, in `tool.py`'s mapping, or in the three registries this change deletes. They are the reason the totality test scans the source tree instead of checking a hand-written list: a list would have been written from the same documents that omitted them.

**Inert-key rule** (the substitute for `rvExemptReasons`, and the reason `INV-ARCH-06` dissolves): a sub-parameter key explicitly present while its owning feature is **inactive** is accepted **iff its value is the feature's declared neutral value** (e.g., `ape.llmPercentageNoSubstrate=-1`, which `_BASELINE_ARM_FLAGS` pushes on every arm including non-LLM ones — verified `tool.py:243-261`); it is recorded in the echo's `inert` list and enters no params object. A **non-neutral** value for an inactive feature is a missing-dependency abort (you may state OFF for an absent mechanism; you may not state ON). This keeps the campaign arms' `ape.properties` valid — checked in `RunSpecCompatTest` against the actual `_push_properties` output of **five** arms: the four campaign arms (`sata`, `sata_mop_widget`, `sata_llm`, `sata_mop_llm`) and the `ape_pure` control, each generated by executing the producer rather than by transcribing its inputs. The other 24 arms are **not** covered by a fixture, and the scope is deliberate (task 6.1): the arms this study runs and the control it reads them against are pinned, and the `bfs`/`dfs` variants and the remaining ablations are stage-5 work — stage 5 replaces these fixtures with the real `preset + overrides` contract anyway.
...
**Three features activate on every campaign arm from jar defaults that no arm dictionary states** (measured at apply time, group 6, and the reason a vector must be read off the resolved plan rather than off `tool.py`). `COVERAGE_BOOST` (`ape.coverageBoostWeight`, default `100`) and `FUZZING` (`ape.doFuzzing`, default `true`) are active on every arm, including the `ape_pure` control; `LLM_RANDOM` is active on both LLM arms, because `ape.llmPercentage` defaults to `0.02` and `_LLM_FLAGS` does not state it — so `sata_llm` and `sata_mop_llm` route to the LLM at a 2% random rate that is invisible from the harness side. None of the three keys is a member of the harness's `ARM_DEFINING_KEYS`, so no arm can turn one on or off relative to another; all three are arm-neutral and cancel in a paired comparison. `RunSpecCompatTest` pins them, as the named set `ARM_NEUTRAL_DEFAULTS` and as `randomLlmRoutingIsInheritedFromTheJarDefault`.

The consequence generalizes past these three keys: **what an arm states is not what the run does.** Any claim about an arm's configuration derived from `tool.py` alone is incomplete, because the arm dictionaries are silent wherever a jar default already says what the arm wants. Read the configuration off the resolved plan — which is what `RUN_START` now puts in the trace.
...
**What "before" binds to** (settled 2026-08-04). The hazard is a jar meeting a `tool.py` that still pushes the key, so the binding event is the moment a post-stage-2 jar is **installed into `aperv-tool`** — `mvn install`, whose `copy-jar-to-aperv-tool` execution overwrites `rv-android/modules/aperv-tool/src/aperv_tool/tools/aperv/ape-rv.jar`. What must hold at that moment is that the rv-android tree in use carries the removal. A git merge is not itself the hazard boundary; it is the durable way of making the condition true on every branch rather than on the one that happens to be checked out.

The distinction became load-bearing when the counterpart's branch policy changed. This document originally leaned on the roadmap's carve-out, under which `gh93` merged into `modules` early and separately, ahead of the rest of the `rearch-counterparts` line. The owner withdrew that carve-out: **`rearch-counterparts` merges into `modules` only after all five counterparts (`gh93`…`gh97`) are finished**, so `gh93` lands with the line rather than before it. The constraint survives unweakened, restated against the binding event: **the `rearch-counterparts` → `modules` merge SHALL precede the first `mvn install` of a post-stage-2 jar.** Both are deliberate end-of-line acts — the `rearch` → `master` merge is likewise post-stage-7 — so this orders two decisions, and gates none of the stage's day-to-day work.

Two consequences worth stating, because reading the constraint as "merge first, then proceed" would deadlock the stage. First, **no group of stage 2 is blocked by the merge**: task 6.0's deliverable is the edit itself, which is done when it is committed in `rvsec`, and group 6 builds nothing and deploys nothing. Second — settled when the device smoke was descoped (D-13) — **no task of stage 2 installs the jar at all.** The gate used to be recorded at the group-8 smoke, which was the one act of the stage that would have put a post-stage-2 jar in front of a device; with that group descoped, stage 2 is wholly jar-side plus one Python edit and deploys nothing. The condition therefore holds **vacuously across the entire stage**, and the binding event sits outside this change, at the first `mvn install` of a post-stage-2 jar — now the coordinated end-to-end smoke. This strengthens the safety argument rather than weakening it: there is no longer any stage-2 act capable of violating the ordering. It also holds in fact, since the rv-android tree runs from `rearch-counterparts`, which carries the removal (`tool.py` sha256 `aba920ea…c93ae8`).
...
### D-13 — The stage-2 device smoke is descoped to a coordinated end-to-end run

Owner decision, 2026-08-04. Stage 2 performs **no device validation**. The smoke that group 8 would have run — one `sata_mop_widget` run through `rv-experiment`, plus the two fail-fast aborts — is folded into a **single coordinated end-to-end run once every planned change on both sides has landed**: the seven `ape` re-architecture stages and the five `rvsec` counterparts (`gh93`…`gh97`).

**Why.** The Python side is still being implemented. A device smoke of a stage-2 jar would validate a moving jar against a moving harness, and every later stage overwrites the jar it installed — so the check would certify an artifact that no campaign will ever run. Seven partial smokes against seven superseded jars are worth less than one smoke against the jar that ships.

**What is deferred** (enumerated in the group-8 disposition block of `tasks.md`): the abort *composition* in `Monkey.run` — including that an unknown key exits nonzero with `[APE-RUNSPEC-ABORT]` injecting zero events, and that `--ape bfs` aborts naming the valid set; the runtime ordering of `RUN_START` ahead of every `[APE-*]` line; the after-state of a real run (no `sataModel.obj`/`sataGraph.*`; coverage dump before `action-history`); and the end-to-end run of the four campaign arms and `ape_pure`.

**What it costs, stated plainly.** Stage 2 ships validated by 956 JVM tests and the per-preset parity goldens, and by **nothing that ran on a device**. The three device-only properties above are asserted by construction and *unverified in execution* until the coordinated run. The exposure is bounded by what the JVM does cover — `RunSpec.resolve` is tested across every abort class, and the echo's content, single-line guarantee and bootstrap position are all guarded — so what remains untested is the composition and the process-level exit, not the logic they compose.

**What it buys.** Stage 2 deploys nothing, so the D-4 ordering has no stage-2 act that could violate it and holds vacuously (see D-4). And one device campaign validates all seven stages against the jar that will actually run, rather than seven partial ones each against a jar the next stage supersedes.
...
- [Stage-2 jar deployed against a pre-edit `tool.py`] → every arm aborts before step 1: this is the failure the D-4 ordering exists to prevent, and it is the reason the counterpart change is a predecessor rather than a parallel task. Residual exposure is an operator deploying the jar out of order — concretely, running `mvn install` while the rv-android tree sits on `modules`, which does not carry the removal until the counterpart line merges; the failure is loud and immediate (a failed task, never a degraded run — R5), and `RUN_START.build.sha` makes the jar-to-trace pairing auditable post-hoc.
...
| Compat | 5 generated fixtures = exact `_push_properties` output of the 4 campaign arms + `ape_pure`, **after the D-4 edit** → all five resolve; 2 hand-written negative fixtures carrying `ape.apePureMode` or `ape.mopWeightActivity` abort with the documented reason. The other 24 arms carry no fixture (task 6.1's deliberate scope) | fixture files in test tree | ~8 |
...
| Device — **descoped from this stage by D-13** | what the check *is*, for whoever runs it: a smoke driven through the platform (`uv run rv-experiment run --tools aperv:sata_mop_widget …`), which owns the whole emulator lifecycle — start, boot wait, APK install, execution, cleanup: RUN_START present and well-formed before any `[APE-*]` line; unknown-key abort observable; no `sataModel.obj` produced | deferred to the coordinated end-to-end run | 0 in this stage |

## Open Questions

None. Owner decisions D1–D6 are final (report Sec. 12). Both details left to apply time are now **closed**:

1. The key → owner classification is enumerated in code and locked by the totality test (D-2, 121 keys, and it found two the documents had missed).
2. `Graph.printDot`/`printVis` were confirmed caller-less outside `saveGraph` by group 5, by grep over `src/main` and `src/test` (D-10). The dead set came to **seven** methods, not two: `printDot`, `printVis`, and the five helpers `printVis` alone called — `getColor`, `getStateTransitionType`, `getNodeTitle`, `getStateTransitionTitle`, `escape`. All seven are deleted. `printStateTransitions` is caller-less too but was already so before this change, so it was deliberately left alone rather than folded into a deletion this change cannot claim.
