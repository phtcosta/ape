# activity-budget — delta: rv-scoring-pipeline

## Purpose

Gate the `ActivityBudgetTracker` behind the `activityBudgetEnabled` parity flag (declared by the `scoring-pipeline` capability; default `true`), so the `ape_pure` arm reproduces upstream APE, which has no per-activity iteration budget. When the flag is `false`, no tracker is instantiated and the budget check in SATA selection is skipped. Default `true` preserves current behavior. The kill-switch (`apePureMode`) additionally restores `activityStableRestartThreshold` to its upstream-inert value (`Integer.MAX_VALUE`), per the `scoring-pipeline` capability.
