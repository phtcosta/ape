...
The cadence (`ape.activityTriggerStagnationStep`, default `50`; a configured value `<= 0` clamped to the default at plan resolution, logged) and the per-run cap (`ape.activityTriggerMaxPerRun`, default `0` = unlimited; `< 0` clamped to `0`, logged) SHALL be injected into the stage from the resolved plan at assembly; the stage SHALL NOT read static `Config` (`decision-pipeline` INV-DP-12). Only actually returned `EVENT_TRIGGER_ACTIVITY` actions consume budget; a firing whose candidate scan comes up empty does not. Sustained exploration yields periodic firing points every cadence steps *that reach the stage*, independent of graph growth; expected launches per run = `min(maxPerRun, non-preempted steps/cadence, |unvisited eligible census|)`.
