...
- [Killing `LlmRouter` breaks its 40+ existing unit tests] → tests migrate unit-by-unit with the code they pin (mapping table above); the repair-pipeline tests (INV-LLM-10/INV-RTR-14) must pass unmodified in assertion content.
...
- [Scoring-params injection changes a default silently] → `ScoringParams` is populated from the same resolved plan the `RUN_START` echo prints; golden equality plus the existing pass unit tests pin every weight.
