...
7. **`ScoringPipeline`'s config parameter is decorative** (V10): `ScoringPipeline.fromConfig(Config cfg, ScoringContext ctx)` (`ScoringPipeline.java:51-61`) — the javadoc `:48-49` admits `cfg` is "present for signature fidelity"; the sole caller passes `null` (`StatefulAgent.java:208`); the passes read `Config` statics directly (`CoveragePass`/`FormCompletionPass`/`FrontierPass`/`MopFrontierPass`/`WtgPass`, plus `MopScorer.mopWeightDirect/Transitive/OpenMenu`). Finding 3.3-3: the class javadoc says "**six passes**" (`:14`, `:43`) while `fromConfig` constructs **seven** (`:52-59`, `MopFrontierPass` included).
...
|---|---|---|
| `SataAgent` (25) | `backMenuPickCap`(5), `componentPercentage`(3), `minEpsilon`/`maxEpsilon`/`defaultEpsilon`/`dynamicEpsilon`, `activityTriggerStagnationStep`(2), `activityTriggerMaxPerRun`(2), `useActionDiffer`, `trivialActivityRankThreshold`, `mopTargetPickCap`, `llmPercentage`, `graphStableRestartThreshold`, `fillTransitionsByHistory`, `fallbackToGraphTransition`, `doBackToTrivialActivity`, `activityBudgetEnabled` | `ExplorationParams` (epsilon family, caps, chain toggles) read via `StepContext`; launcher pair → `MopLauncherStage` ctor; `componentPercentage` → assembly + `ComponentTriggerStage`; `activityBudgetEnabled` → assembly; `llmPercentage`/`graphStableRestartThreshold` → LLM stages |
...
| Scoring passes (9) | `coverageBoostWeight`(2), `formCompletionEnabled`, `frontierBoostWeight`(2), `mopFrontierWeight`(3), `mopWeightWtg` | `ScoringParams` via pass constructors (D8) |
| `MopScorer` (7) | `mopWeightDirect`(2), `mopWeightTransitive`(2), `mopWeightOpenMenu`(3) | parameters supplied by `MopWidgetPass`/`MenuGatewayPass` from `ScoringParams` (D8) |
| `StatefulAgent` (~30) | throttles, restart thresholds, save/screenshot flags, `stepTelemetryEnabled`(3), `evolveModel`, `mopDataPath`(2), `llmUrl`, budget params, `maxIdleTimeoutMs`(2), … | Decision-relevant ones (`activityStableRestartThreshold`, `graphStableRestartThreshold`, `stateStableRestartThreshold`, budget params, `evolveModel`, throttles) → `ExplorationParams` via `RunContext`; `llmUrl`/`mopDataPath` construction reads were already replaced by rearch-02 plan resolution; telemetry/save flags → `TelemetryParams` (consumed at emission sites, restructured anyway in stage 4) |
...
- [Killing `LlmRouter` breaks its **66** existing unit tests] → counted at HEAD, not estimated: `LlmRouterTest` 31, `LlmRouterDeadPairTest` 12, `LlmRouterCoordinateMappingTest` 11, `LlmRouterMappingTest` 5, `LlmRouterToolSchemaTest` 4, `LlmRouterTelemetryTest` 3; `CoordinateMapIntegrationTest` and `SglangClientTest` also reference the class. Tests migrate unit-by-unit with the code they pin, per the per-file destination table in task 4.7 (the responsibility table above maps *code*, which is not the same mapping); the repair-pipeline tests (INV-LLM-10/INV-RTR-14) must pass unmodified in assertion content.
