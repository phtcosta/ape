...
Deleted (writer sites in Context §5): `saveGraph()`'s dot/vis/per-state branches and the `Graph.printDot`/`Graph.printVis`/`State.saveState` methods they call; `saveActionHistory()` + `Model.saveActionHistory`; the produce/consume `PrintWriter`s, their five `ApeRRFormatter.log*` call sites, and the `sataTimeline.vis.js` export (`toVisTimeline`); Config flags `saveDotGraph`, `saveVisGraph`, `saveStates` (and `stepTelemetryEnabled`, whose gate dies with always-on telemetry); the `[APE-STEP]`/`[APE-OUTCOME]`/`[APE-LLM-TEL]`/`[APE-LLM-ERROR]`/`[APE-LLM-PROMPT]`/`[APE-LLM-RESPONSE]`/`[APE-LLM-CONFIG]`/`[APE-LLM-CONFIG-ACK]`/`[APE-MOP-DATA]`/`[APE-ARCH]`-passes emitters. **Kept**: `ApeRRFormatter.readActions`/`readAction` and everything `ReplayAgent` needs (`ReplayAgent.java:63`) — replay consumes externally supplied logs; only the tool's own production of those logs ends.

**The caller audit this decision called for has now been run (task 7.1, 2026-08-05), and it moved two things out of the "kept" column — both owner-decided at apply time.** This section is corrected rather than annotated, because a design that names a method as preserved while the code deletes it is worse than one that never mentioned it.

- **`ApeRRFormatter.parseRect` is deleted, not kept.** The first draft of this section paired it with `readActions` on the assumption that the replay path parses bounds. It does not: `ReplayAgent` reads `actionType`, `target`, `full` and `throttle` from the parsed log (`ReplayAgent.java:134-152`) and never touches `bounds`, and `ApeRRFormatter.parseRect` (`:153`) has zero callers anywhere in `src/` — `GUITreeBuilder` parses bounds with its own `parseRect` (`:86`, used at `:499`), which is what feeds the real GUI-tree path. Keeping it would have preserved a method by reputation rather than by use, against P3. `readActions` genuinely is needed and stays, together with `readAction` (`:170`), which it calls.
- **The `toJSONObject` family dies with the produce log.** This is the question the section delegated, and the audit answers it: the family's only external entry point is `ApeRRFormatter.recordAction` (`:182`), reached only from `startLogAction` (`:159`), reached only from the produce-log writers this change deletes (`MonkeySourceApe:1220` via 7.3, `Model.java:116` via 7.2). Nothing in the replay or XPath paths reaches it — `readAction` parses JSON with `org.json` directly and constructs nothing. So the family is not "shared with replay"; it is the serializer half of a log format whose writer is being removed, and it becomes unreachable the moment 7.2 and 7.3 land. Deleted with them: `formatRect` (`ApeRRFormatter:149`, whose sole caller is `ModelAction.toJSONObject` itself), `recordAction`, and `toJSONObject` on `Action:248`, `ModelAction:398` (the stale `:344` of the first draft), `CrashAction:35`, `StartAction:76`, `FuzzAction:71`, `Crash:58`, `AbstractApeEvent:30`, the `ApeEvent:28` interface method, and the seven `Ape*Event` implementations.
