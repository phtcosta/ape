...
| `[APE-MOP-DATA] status=...` (`MopData.java:203-332`) | `{"type":"MOP_DATA","status":"loaded"\|"rejected","reason":...,"package":...,"windows":...,"widgets":...}` | same statuses/reasons |
| `[APE-ARCH] passes=[...]` (`ScoringPipeline.java:36`) | `{"type":"PIPELINE","stages":[...],"passes":[...]}` | assembly provenance; stage list from rearch-03 |
...
| (nothing today) | `{"type":"RUN_END","reason":...,"steps":N,"counters":{...}}` | reason ∈ `timeout` (StopTestingException/time budget), `crash` (abnormal Throwable, class name in `detail`), `unknown`. Write-only, D5: **no consumer validates it** |
...
All `EventSink` methods return `void` and take primitives/strings the caller already has; the sink never hands anything back to decision code, never touches the seeded RNG, and holds no reference to `Model`/`Graph`/`RunContext` state — it interns copies (strings, ints) only. `NoopSink` is the test double; `TelemetryParams.sink` exists **only** so the neutrality test can construct the off configuration — every preset pins it on (telemetry is always-on for all arms; that is the point of dissolving INV-ARCH-01). The R7 test runs the rearch-01 parity harness with `NdjsonSink` vs `NoopSink` under the same seed and asserts identical action sequences.
