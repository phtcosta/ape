# component-triggering Delta Specification

## Purpose

Delta for `rearch-04-step-ndjson-telemetry`, over the one `component-triggering` requirement that
normatizes a retired rendering by name.

INV-CT-07 states the launch's model-visibility as "exactly one `[APE-STEP]` line with
`decision_source=Component`" — the line family this change retires (task 3.1). Nothing about
component triggering changes here: the cadence and its exact-equality gate, the census eligibility,
the framework denylist, the budget accounting, the RNG-draw position and every other clause are
carried forward from `rearch-03-decision-pipeline` verbatim, that being the text this delta is
written over since the predecessor archives first. Only the rendering the invariant names moves,
from the `[APE-STEP]` line to the step record's `dec.src`.

**Why this capability needed a delta at all, when the launch mechanism is untouched.** A requirement
that names a rendering is a requirement about that rendering: synced unchanged, INV-CT-07 would
demand a line no code emits, and the arm whose strongest mechanism result it guards — roughly 1,075
direct component launches in the guided arm against 0 in the control — would be the one asserted
against a format that does not exist. The invariant is cheap to re-anchor and expensive to leave.

The launch also gains something it never had, named here because this is where a reader looks for
it: `dec.comp` carries whether the platform accepted the intent. The retired line could not carry
it — it was emitted *before* dispatch — so a refused launch and an accepted one were the same trace
evidence. The field and its two scenarios are specified in the `event-sink` capability (`dec.comp`,
"launch refused by the platform") and are deliberately not restated here, so the dispatch contract
...
- **INV-CT-07 (re-anchored, mechanism unchanged)**: every launch model-visible as exactly one `StepRecord` carrying `dec.src:"Component"`; no graph edge labeled by the launch. Model-visibility is the property the invariant protects; the retired line was only the rendering that carried it, and `event-sink` INV-SNK-03 supplies the same one-record-per-step counting that made "exactly one" checkable in the first place. The record additionally carries `dec.comp` — evidence the retired rendering could not hold, because it was emitted before dispatch while the record closes at step N+1.
