# llm-prompt Delta Specification

## Purpose

Delta for `rearch-04-step-ndjson-telemetry`, over one clause.

The widget-list requirement flattens embedded newlines in a widget's identifier text and gives two reasons: keeping the element list per-line parseable, and keeping the `[APE-LLM-PROMPT]` dump per-line parseable. This change retires `[APE-LLM-PROMPT]` — prompts and responses travel as JSON-escaped `sys`/`user`/`resp` fields on the step record's `llm[]` sub-event, where a newline is simply an escaped character.

**The flattening itself stays**, and the clause is narrowed rather than deleted: the element list is a line-oriented format read by the model, and one action per line is a property of the prompt, not of the trace. What ends is the second reason — the trace no longer needs the data bent to fit it, which is the whole point of escaping by construction (INV-SNK-02).
