...
> Ordering: this change PRESUPPOSES the archive of the six open changes (`activity-frontier`, `back-menu-pick-cap`, `sibling-state-depriority`, `foreign-activity-guard`, `tree-package-guard`, `idle-timeout-cap`). Archive them first; the deltas here are written against post-archive main.
...
- [ ] 2.3 `ScoringPipeline.fromConfig(Config, ScoringContext)` — single assembly point; constructs the seven passes in fixed order, retains only `isEnabled()`==true, emits one `[APE-ARCH] passes=[...]` line (INV-ARCH-03, INV-ARCH-04)
...
- [ ] 3.6 `SiblingPenaltyPass` — extract `:1610-1635` (the post-archive `sibling-state-depriority` penalty); `isEnabled = Config.siblingStatePenalty > 0`
- [ ] 3.7 `FormCompletionPass` — extract `:1640-1660`; `isEnabled = Config.formCompletionEnabled` (**new** flag, task 5.1)
- [ ] 3.8 Rewrite `StatefulAgent.adjustActionsByGUITree()` to the upstream base-priority loop (byte-identical to `ape @ 8f51b99`) + one `pipeline.apply(state, actions, ctx)`; hold one `ScoringPipeline` field built once at construction (INV-ARCH-05)
- [ ] 3.9 Per-pass "disabled = strict no-op" tests: a disabled pass is absent from the pipeline and mutates nothing (INV-ARCH-02)
...
- [ ] 7.6 Device smoke (rebuilt jar) on `cryptoapp` for the `ape_pure`, `sata`, and a MOP arm: `ape_pure` emits `[APE-ARCH] passes=[]`, no `[APE-STEP]`, no `MODEL_MENU`; `sata` emits the coverage/sibling/form passes; the MOP arm emits the full ordered set — confirming the single binary composes each arm by properties alone
