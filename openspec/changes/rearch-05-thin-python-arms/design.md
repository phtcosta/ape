...
|-------------|---------------|------|
| Tool Variants (MODIFIED: preset + overrides) | `ApeRVTool.get_variants()` | restated structural tests in `test_aperv_tool.py` (names, aliases, override dicts) |
...
- `sata_mop_widget` / `sata_mop` (alias, same object) → `preset: "mop"`, no overrides.
