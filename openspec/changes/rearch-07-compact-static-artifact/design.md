...
- [Derivation semantics drift from what the old parser did] → the corpus equivalence gate runs both parsers over 134 real JSONs pre-cutover; per-rule unit tests pin each relocated invariant permanently on the Python side.
...
| Equivalence (one-shot gate) | old-parser(full) ≡ new-parser(derived) on widget maps, both activity sets, gateways, WTG views, trigger/provider tuples, scalars | JVM test over `data/instrumented_apks/` via `-Dmop.corpusDir`; deleted with the old parser after green | 1 × 134 apps |
