...
**Reason**: Superseded by the MODIFIED "MopData — Load Status Line and Fail-Fast" requirement of this change, whose field set now includes `mopActivities=<n> mopActsAugmented=<n>` sourced from the loaded wire sets.

**One number changes meaning, and it is stated rather than absorbed.** An earlier revision of this note claimed that "counters over the load" and "counters over the wire sets" are *the same numbers* because the augmentation is host-side. That is false on any run with `mopActivitySourceComponents == false`: the retired counter reported the entries the augmentation *applied*, which is 0 whenever the flag is off, while the wire ships both sets whole and their difference is flag-independent. `mopActsAugmented` now means **`|mopActivitiesAugmented \ mopActivities|`** — what the augmented source would contribute, whether or not this run selected it — and SHALL NOT be recomputed as a function of the selected set (owner decision 2026-08-05). The applied augmentation remains recoverable from the trace, because `RUN_START` independently carries the flag in its feature list and its `params` echo; the availability fact has no other carrier, which is why the wire-set reading is the one kept. Consequence for anyone diffing pre- and post-cutover traces of a flag-off run: this field goes from 0 to N, and that is the field working, not a regression.

INV-MOP-32's discipline — counters are pure observability and never change sets, flags, or the load outcome — is preserved and restated in the modified requirement's stats-echo clause, and it is what makes the redefinition safe: no set, flag or load outcome reads this number. The standalone requirement is folded, not weakened.
