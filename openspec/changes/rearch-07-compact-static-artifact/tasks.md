...
- [ ] 2.2 Implement flag derivation: `bySignature` index from `reachability[]` (duplicate signatures merged by OR, not last-write), producer-precedence, D8 synthetic-lambda recovery, per-normalized-eventType `none|direct|transitive|both` encoding, and `transitive = reachesTarget OR direct` on every path so `direct` implies `transitive` (INV-DRV-01). The two axes stay independent: `direct` keeps the producer's 0-hop meaning and is **not** redefined as any-depth reach the way the retired `INV-APV-32` enrichment did (design D10)
