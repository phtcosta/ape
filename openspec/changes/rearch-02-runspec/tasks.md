...
- [ ] 4.3 Create `RunSpecEcho.emit`: `RUN_START` object per design D-7 (`type,v,run_id,seed,agent,preset,features,params,inert,corpus_basis,digest,props_digest,build`); wire into `Monkey.run` immediately after `RunContext.initialize`, before `MonkeySourceApe` construction
- [ ] 4.3a Register `ape.corpusBasis` as a recognized resolver-owned key (beside `ape.runId`) and echo it as `corpus_basis`, omitted when absent, read by nothing (INV-RUN-03). It exists because this study has counted its analysis basis as 163, 181 and 219 applications in different documents; one hash per run ends the re-derivation. Do **not** add per-application static facts here — the basis is a property of the corpus, not of the plan, and per-app copies would write the same truth in forty places
- [ ] 4.3b Record in the verification notes that the seed field will be empty under the current deployment: the harness appends `-s` only when a seed is configured and none is, so `RUN_START` will faithfully record the *absence* of a seed (verified: the string occurs in no trace of the decisive campaign except as a widget resource id, and none of the 115 echoed `ape.*` keys is a seed/RNG key). Supplying one is a `rearch-05` item, and it SHALL be a deterministic function of (application, arm, replica) — never one constant for the campaign, which would collapse the three-replica variance estimate that every effect size is read against
