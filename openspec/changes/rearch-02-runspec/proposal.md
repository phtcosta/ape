...
- Presets (`aperv`, `mop`, `llm`, `llm_mop`) reside **in the jar** (`Presets.resolve`); overrides come explicitly on top. **Zero Python changes in this stage** — `tool.py` continues pushing the same properties it pushes today.
...
- Level-0 configuration echo (owner decision D1): `RUN_START` as the first trace line, carrying the effective plan, the digest of the properties actually read, and the jar build version. Write-only provenance; **no automatic validation anywhere, zero Python changes**.
...
- **Python/rv-android**: none (D1 — level 0 is definitive; drift auditing is post-hoc analysis).
