...
| run-spec: Total Fail-Fast (INV-RUN-02) | validator + `Monkey.run` abort path | `RunSpecAbortTest` (unknown key / bad type / dependency / combination / retired key); the abort *composition* in `Monkey.run` is device-side, task 8.2 |
...
| run-spec: single seeded stream (INV-RUN-08) | `RunContext.initialize` seeds `RandomHelper`; `StringCache` draws from it | `DeviceInputChannelAbsenceTest.noUnseededGeneratorSurvivesInProductionCode` (`ThreadLocalRandom` only — see the coverage note in the run-spec delta); `StringCacheSeededTest` |
| run-spec: no read-back of a previous run (INV-RUN-07) | `Graph.readGraph`, `--ape-model` and `ape.modelFile` deleted | compile-enforced (no caller can name the deleted API) + `RunSpecAbortTest.everyRetiredKeyAbortsWithItsOwnReason`; **no absence guard exists** — see the coverage note in the run-spec delta |
...
Two consequences worth stating, because reading the constraint as "merge first, then proceed" would deadlock the stage. First, **no group of stage 2 is blocked by the merge**: task 6.0's deliverable is the edit itself, which is done when it is committed in `rvsec`, and group 6 builds nothing and deploys nothing. Second, the group-8 device smoke is the first task that puts a stage-2 jar in front of a device, so it is where the gate actually binds — recorded there rather than left implicit. Until one of those two events happens the deployed jar is untouched and the condition holds vacuously; it also holds in fact, since the rv-android tree runs from `rearch-counterparts`, which carries the removal.
