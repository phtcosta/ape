...
- `node.getInputText(): String` — current text on an `EditText` node; `null` means unfilled.
...
The system SHALL determine whether the current state carries at least one unfilled `EditText` field. An action counts as an unfilled `EditText` when it requires a target (`requireTarget()`), is valid (`isValid()`), is resolved at the current timestamp (`isResolvedAt(timestamp)`), its resolved node `isEditText()`, and the node's `getInputText()` is `null`. This predicate defines the "form-completion context" and SHALL be computed from `State.getActions()` without mutating any action. The same predicate SHALL be read by both the form-completion boost pass and `ApeAgent.checkInput()`.
