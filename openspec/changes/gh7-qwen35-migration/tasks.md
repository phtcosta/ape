...
- [ ] 1.1 Add `llmEnableThinking` (boolean, default `false`) and `llmImageResize` (boolean, default `false`) to `Config.java`
...
- [ ] 2.1 Add `enableThinking` field to `SglangClient`:
...
- [ ] 2.2 Handle comma-separated string coordinate format in `SglangClient.putPrimitive()`:
...
- [ ] 2.3 Update existing `SglangClientTest.makeClient()` to pass 8th param (`false`):
  - All existing tests instantiate via `makeClient()` — update that single factory method
- [ ] 2.4 Add new unit tests for SglangClient changes:
...
- [ ] 3.1 Add `FIX_STRING_COORDS` regex pattern to `ToolCallParser.fixMalformedJson()`:
...
- [ ] 3.2 Add unit tests for ToolCallParser comma-separated string fix:
...
- [ ] 4.1 Update `ImageProcessor.processScreenshot()` signature to accept `boolean resize`:
...
- [ ] 4.2 Update existing `ImageProcessorTest` @Ignore tests to use 2-param signature:
  - `processScreenshot(pngBytes)` → `processScreenshot(pngBytes, true)` in all 4 @Ignore tests
- [ ] 4.3 Run `/sdd-test-run`

## 5. ApePromptBuilder — Screen Dimensions in System Message

- [ ] 5.1 Update `ApePromptBuilder.buildSystemMessage()` to accept `int deviceWidth, int deviceHeight`:
...
- [ ] 5.2 Add unit test for screen dimensions in system message:
  - `testSystemMessageContainsScreenDimensions`: verify `"Screen: 1080x1920 pixels."` present in output
- [ ] 5.3 Run `/sdd-test-run`

## 6. Wire Changes in LlmRouter

- [ ] 6.1 Update `LlmRouter()` constructor to pass `Config.llmEnableThinking` as 8th param to `new SglangClient(...)`
  - File: `src/main/java/com/android/commands/monkey/ape/llm/LlmRouter.java`, line 79-86
- [ ] 6.2 Update `LlmRouter.selectAction()` to pass `Config.llmImageResize` to `imageProcessor.processScreenshot(pngBytes, Config.llmImageResize)`
...
- [ ] 6.3 Update CLAUDE.md: add `llmEnableThinking` and `llmImageResize` to Config flags table

## 7. Build and Verification

- [ ] 7.1 Run `mvn compile` — verify no compilation errors
- [ ] 7.2 Run `/sdd-test-run` — all tests pass (including updated existing tests)
- [ ] 7.3 Run `/sdd-qa-lint-fix`
- [ ] 7.4 Run `/sdd-verify`
- [ ] 7.5 Run `/sdd-code-reviewer`
- [ ] 7.6 Package `ape-rv.jar` for Docker image rebuild
