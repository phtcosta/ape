...
**Goals:** foreign screens deflected with one light BACK and excluded from model/UICOV; permission dialogs not deflected by the guard (best-effort, one-cycle — see Decision 4); first JVM test coverage for the package decision; dead `checkPackage` deleted.
...
- [Whitelist incomplete for some Android build (OEM permission UI)] → symptom is a BACK on a permission dialog; note Monkey's auto-grant (`Monkey.java:404-414`) covers **only** the legacy `com.android.packageinstaller`/`GrantPermissionsActivity` flow, not `com.android.permissioncontroller` nor the Google package, so it does not backstop the modern permission UI — extend the whitelist set if a validation run shows a build whose permission package is missing.
