...
### Input
- `ape.maxIdleTimeoutMs: long` — default `10000`; the global timeout (ms) of the UiAutomation idle wait in `getRootInActiveWindowSlow`, and (÷1000) the seconds threshold of the `refreshNewState` "window stuck animating" retry break. The default preserves current behavior exactly.
...
The `refreshNewState` retry-loop "window stuck animating" break threshold SHALL be derived from the same flag as `maxIdleTimeoutMs / 1000` seconds, so that lowering the ceiling keeps the break firing. With `ape.maxIdleTimeoutMs` at its default `10000`, the idle-wait global timeout SHALL be `10000` ms and the break threshold SHALL be `10` seconds — byte-identical to the pre-change literals (`1000 * 10` and `>= 10`).

- **INV-EXPL-23**: With `ape.maxIdleTimeoutMs` at its default `10000`, slow tree capture and the `refreshNewState` break SHALL behave identically to the pre-change implementation (the global timeout is `10000` ms and the break threshold is `10` s).
...
- **INV-EXPL-25**: The `refreshNewState` break threshold SHALL be derived from `ape.maxIdleTimeoutMs` (as `maxIdleTimeoutMs / 1000` seconds), never from an independent literal, so the ceiling and the break cannot diverge.
...
- **THEN** the UiAutomation idle wait SHALL use a `10000` ms global timeout, the `refreshNewState` break SHALL fire at `>= 10` seconds, and the observable behavior SHALL be identical to the pre-change implementation
...
- **THEN** the idle wait SHALL return after at most `2000` ms, `getRootInActiveWindow()` SHALL still be called and its tree returned (no capture is skipped), and the `refreshNewState` break SHALL fire at `>= 2` seconds
